...
const bookDetailHandler = async (
  ...
  switch (req.method) {
    case "GET":
      try {
        res.status(200).json(await getRoomDetail(req));
      } catch (err: any) {
        console.error(err);
        res.status(500).json({
          message: err.message,
...
async function getRoomDetail(req: NextApiRequest) {
  // Get roomId;
  ...
  const book: any = await prisma.room.findUnique({
    where: {
      id: roomId,
    },
    include: {
...
async function updateBookDetail(
  req: NextApiRequest,
  res: NextApiResponse<any>,
) {
  // Get roomId;
